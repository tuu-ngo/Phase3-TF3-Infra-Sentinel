...
- **Tooling**: Sử dụng Trivy phiên bản cố định qua Github Action (`setup-trivy@81e514348e19b6112ce2a7e3ecbafe19c1e1f567`).
...
- Pipeline đảm bảo các Image có điểm rủi ro chưa xử lý sẽ không vượt qua được CI Gate để tiến đến production.

## 6. Runtime kubernetes Validation
- Kiểm tra Pod tại Namespace `techx-tf3` sẽ xác nhận toàn bộ container `imageID` đang map đúng với digest đã định nghĩa trong `values-prod.yaml` và `values.yaml`. 
