...
| Secrets Manager × 2 | $0.40/secret-mo | **0.80** |
| **TỔNG** | | **≈ $201.76/mo ≈ $46.6/tuần** |
...
### 2. TLS gated bằng env — tách "deploy code" khỏi "cutover"

Sửa 4 service để TLS **có công tắc env**, mặc định **tắt**:

| Service | Biến | Mặc định | Khi bật |
|---|---|---|---|
| `cart` | `VALKEY_TLS` | `false` | `ssl=true` trong `ValkeyCartStore` |
| `checkout` | `KAFKA_TLS_ENABLED` | `false` | `saramaConfig.Net.TLS.Enable = true` |
| `accounting` | `KAFKA_TLS_ENABLED` | `false` | `SecurityProtocol = Ssl` |
| `fraud-detection` | `KAFKA_TLS_ENABLED` | `false` | `SECURITY_PROTOCOL_CONFIG = "SSL"` |

`cart` cần **thêm một thay đổi nữa** ngoài TLS: nhánh **dual-write** tạm (`VALKEY_DUAL_WRITE_ADDR`,
mặc định rỗng = tắt) — điều kiện của chứng minh §5. Tổng cộng: **4 service sửa code, `cart` sửa 2 việc.**
...
### 3. Credential → Secrets Manager, render 3 format

- 1 secret nguồn (`techx-tf3/postgres`) chứa `username`/`password`/`host`/`dbname`.
- Đưa vào pod bằng **External Secrets Operator** (hoặc Secrets Store CSI) → sinh 3 K8s secret với
  **3 format** đúng cho .NET / Go-URL / libpq, inject qua `envFrom`/`valueFrom.secretKeyRef`.
- **Gỡ sạch `otelu/otelp` khỏi `values.yaml`** — đây cũng vá luôn lỗ hổng ADR 0002 đã ghi nhận.
- Chi phí: ~$0.40/secret/mo — không đáng kể.
...
- **Không đụng flagd.** Storefront vẫn public, cổng vận hành vẫn private (Directive #1) — 3 store
  đều đặt trong **private subnet**, security group chỉ mở từ node group EKS, **không public endpoint**.
...
4. **Bảo mật:** endpoint private (không resolve từ ngoài), TLS on (`sslmode=require` / `ssl=true` /
   `SSL`), `values.yaml` **không còn** `otelu/otelp` plaintext.
...
| Kafka | trả `KAFKA_ADDR` + tắt `KAFKA_TLS_ENABLED` → kafka cũ | ~1 phút | message trong MSK chưa consume → phải replay tay |
...
- [x] Verify đơn giá qua AWS Pricing API (16/07) — $201.76/mo ≈ $46.6/tuần
...
- [ ] Terraform: RDS (Multi-AZ, PG 17.6) + ElastiCache (Valkey 9.0, 1 replica) + MSK (3 broker, 3.9.x KRaft) — private subnet, SG chỉ từ node group, encryption at rest
- [ ] Secrets Manager + External Secrets → 3 format conn string; gỡ plaintext khỏi `values.yaml`
- [ ] Code: TLS gated bằng env cho `cart` / `checkout` / `accounting` / `fraud-detection` + build image
