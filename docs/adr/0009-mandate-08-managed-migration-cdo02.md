...
- [ ] Code: TLS **+ auth** gated bằng env cho `cart` / `checkout` / `accounting` / `fraud-detection` + build image
      ...
      - [x] `cart`: source hỗ trợ `VALKEY_TLS` + `VALKEY_AUTH_TOKEN`, mặc định tắt nên chưa đổi hành vi live.
      - [ ] `checkout`: Kafka SASL_SSL/SCRAM.
      - [ ] `accounting`: Kafka SASL_SSL/SCRAM.
      - [ ] `fraud-detection`: Kafka SASL_SSL/SCRAM.
      - [ ] Build/push image và cập nhật `imageOverride.tag`.
- [ ] Code: **dual-write tạm trong `cart`** (`VALKEY_DUAL_WRITE_ADDR`) — điều kiện của chứng minh §5
      - [x] Source hỗ trợ `VALKEY_DUAL_WRITE_ADDR`, `VALKEY_DUAL_WRITE_TLS`, `VALKEY_DUAL_WRITE_AUTH_TOKEN`; lỗi target phụ chỉ log warning sau khi primary write đã thành công.
      - [ ] Deploy image mới với dual-write mặc định tắt.
      - [ ] Bật dual-write trong cửa sổ cutover Valkey.
