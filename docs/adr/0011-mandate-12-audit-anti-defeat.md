...
- Object đã ghi trước upgrade giữ retention cũ (`GOVERNANCE 14 ngày`). Claim M12 chỉ bắt đầu từ cutover timestamp khi selectors mới active, digest healthy và object mới có `COMPLIANCE 365 ngày`.

---

## Decision 2: Object Lock nâng lên COMPLIANCE 365 ngày và lifecycle 400 ngày

**Quyết định chọn:** Đổi default Object Lock cho object **mới** từ `GOVERNANCE 14 ngày` lên `COMPLIANCE 365 ngày`. Lifecycle 400 ngày (35 ngày đệm sau khi Object Lock hết hạn).

### Alternatives considered

| Phương án | Lý do loại bỏ |
|---|---|
| Giữ GOVERNANCE 14 ngày của M11 | Admin vẫn có thể xóa object bằng quyền `s3:BypassGovernanceRetention` — không đạt yêu cầu WORM của Mandate 12 |
| COMPLIANCE với retention ngắn hơn (ví dụ 90 ngày) | Không đủ để điều tra attacker ẩn mình nhiều tuần; không đáp ứng "cuộc tấn công kéo dài" của đề bài |
| Dùng Glacier Instant Retrieval ngay lần đầu | Chi phí transition/retrieval và minimum storage duration chưa có model; chỉ bổ sung bằng change riêng sau cost model dựa trên kích thước object thực tế |

### Consequences and trade-offs

**Positive:**
- `COMPLIANCE` mode: kể cả root user không thể xóa object trong 365 ngày — đây là WORM thực sự, không phải WORM "trên danh nghĩa".
- 365 ngày tạo cửa sổ điều tra 12 tháng, bao phủ phát hiện trễ và review theo quý.
- Lifecycle 400 ngày cho 35 ngày đệm để hoàn tất export, legal/security review và xử lý sự cố trước khi object đủ điều kiện hết hạn.

**Negative / residual risk:**
- `COMPLIANCE` không thể rút ngắn sau khi set; nếu cấu hình sai phải chờ hết retention. Vì vậy, apply phải có saved plan hash và gate review trước.
- Storage cost tăng so với M11 (lifecycle 30 ngày → 400 ngày). Forecast chi phí phải được Data/Security owner ký trước apply.
...
Lambda có Free Tier 1 triệu request và 400.000 GB-s/tháng; heartbeat chỉ dùng ~0.86% request và ~2.7% compute của hạn mức này. Chi phí chính của Mandate 12 nhiều khả năng đến từ **S3 Data Events và lưu log 400 ngày**, không phải heartbeat. Các khoản không nằm trong bảng trên: CloudTrail S3 Object Data Events, storage archive 400 ngày, alert storm nếu heartbeat liên tục FAIL, thuế và chênh lệch giá theo region.
...
- Object Lock COMPLIANCE 365 ngày đã set không thể rút ngắn — phải chấp nhận điều này trước apply.
...
- Object mới có `COMPLIANCE 365 ngày` với evidence `retain-until`.
