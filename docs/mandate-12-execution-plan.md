...
| Heartbeat | Lambda 5 phút/lần kiểm ~30 invariant; 2 alarm; 1 SNS fallback cùng region, mã hoá bằng CMK riêng |
...
**Một câu còn cần Tech Lead quyết** — chi tiết ở §16: ai chịu trách nhiệm điều phối 10 lần xác nhận subscription SNS còn thiếu.
...
| `aws_kms_key.m12_heartbeat_fallback` + alias | CMK mã hoá topic fallback, ngang chuẩn 2 topic M11 (gate PM-126 chặn SNS không mã hoá). Key policy cho `sns.amazonaws.com` khoá theo encryption context, và cho `cloudwatch.amazonaws.com` để alarm action publish được vào topic đã mã hoá |
...
- `+ create`: rule/target/permission cho `g7` và `g8`, toàn bộ resource heartbeat, SNS fallback + policy + 6 subscription, KMS key + alias cho topic fallback, policy CloudWatch cho topic primary
...
> [!IMPORTANT]
> `set-alarm-state` là bài test **duy nhất** chứng minh được đường alarm → SNS primary thông. Hai topic đều mã hoá bằng CMK, nên đường này còn phụ thuộc key policy cấp cho `cloudwatch.amazonaws.com` (`cloudwatch_alarm_publisher_enabled`) — heartbeat **không** kiểm được vế KMS, nó chỉ đọc SNS topic policy. Nếu grant sai thì alarm vẫn chuyển ALARM, SNS không nhận gì, mà mọi invariant vẫn PASS. Cho tới khi test này xanh cho **cả hai** topic, fallback là thứ duy nhất chứng minh có tín hiệu — không gỡ.
