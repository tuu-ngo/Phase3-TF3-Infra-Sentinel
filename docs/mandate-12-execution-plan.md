...
| 6 | `aws_cloudtrail.audit` | Bỏ `event_selector`; thêm 2 `advanced_event_selector`; `prevent_destroy` + `precondition` chống đưa audit bucket vào selector | Bật data events; tránh vòng lặp logging |
...
|---|---|---|
| `audit_detection_s3_data_event_arns` | `[]` | Validation format; xem §8 gate 1 |
...
- `audit_detection_s3_data_event_arns` để **comment sẵn** kèm hướng dẫn — điền trước apply (§8 gate 1)
...
Thêm trigger `pull_request` và nới điều kiện job:
```yaml
...
Bỏ qua zip do `archive_file` sinh: `m12-audit-heartbeat.zip`, `audit-alert-router.zip`.

### 4.11 `infra/bootstrap/github-oidc/ci-audit-boundary.tf` — MỚI

Permissions boundary cho hai GitHub Actions Terraform role. Chi tiết thiết kế ở **§9**.
...
- Thêm biến `ci_audit_boundary_name` và `enable_ci_audit_boundary` (**default `false`**)
...
| Gỡ `gitlab-ci-deployer` khỏi allowlist + rotate key | PR IAM hardening (cần owner GitLab xác nhận downtime) |
| Bật MFA / thu hồi long-lived key | PR IAM hardening |
...
|---|---|---|---|
| 1 | `heartbeat.py` đặt ở `modules/audit-detection/lambda/` | Đặt ở `modules/audit-detection/lambda-heartbeat/` | Module M11 đóng gói router bằng `archive_file` với `source_dir = "${path.module}/lambda"`. Mọi file trong đó lọt vào `audit-alert-router.zip` → mỗi lần sửa heartbeat làm **router redeploy thừa** và sinh `UpdateFunctionCode` → alert g7 vô ích |
...
|---|---|---|---|
| 1 | **Exact S3 coverage được ký** | Data owner phân loại 8 bucket, chọn exact prefix nhạy cảm. Điền `audit_detection_s3_data_event_arns` (đang comment trong tfvars). Mỗi ARN kết thúc `/`, khớp 1:1 với coverage matrix | ❌ |
...
| `sns` | DeleteTopic, **Unsubscribe**, RemovePermission<br>`techx-corp-tf3-audit-detection-*`<br>`techx-corp-tf3-m12-audit-heartbeat-fallback` | CreateTopic, Subscribe, SetTopicAttributes |
...
| `iam` | Put/DeleteRolePermissionsBoundary<br>**chỉ trên 2 CI role**<br>Create/Delete/SetDefault PolicyVersion, DeletePolicy<br>**chỉ trên chính boundary này** | CreateRole, AttachRolePolicy, PutRolePolicy, CreatePolicy… (CRUD chung), và attach boundary cho **mọi** user/role khác |

Hai deny `iam` cuối là điều kiện cần: không có chúng thì CI chỉ việc tự gỡ boundary rồi làm gì cũng được. Chúng **không** cản phase IAM hardening — `PutUserPermissionsBoundary` không bị deny, và `PutRolePermissionsBoundary` chỉ bị deny trên đúng 2 CI role.
...
### Một điểm phải verify bằng simulation

`sns:Unsubscribe` — AWS không nhất quán về resource-level permission cho action này (subscription ARN vs topic ARN). Nếu simulation cho thấy deny **không** bắt, thì bỏ nó khỏi statement và dựa vào hai lớp còn lại: `g7` đã map `sns:Unsubscribe` nên vẫn alert, và heartbeat kiểm tra subscription `Confirmed` mỗi 5 phút. Ghi kết quả vào evidence dù theo hướng nào.
...
**Lưu ý quan trọng:** khi boundary đã attach, **CI không tự gỡ được** (`DenyRemovingOwnBoundary`). Rollback bắt buộc do người có MFA thực hiện tại root bootstrap. Đây là chủ ý — nhưng nghĩa là phải luôn có ít nhất một người giữ được đường đó.

### 9.6 Access Analyzer warning
...
| 1 | Unit test §7 → tạo PR → CI plan chạy trên PR → review | Plan chỉ update/add audit control; **zero destroy/replace** | 2-4 giờ |
...
| 4b | **Attach CI boundary** (§9.4): apply bootstrap tạo policy → simulate → `enable_ci_audit_boundary = true` → apply lại → smoke test CI plan | Bảng A và B đều rỗng; CI plan chạy hết không `AccessDenied` | 60-90 phút |
...
Tạo canary secret + canary object trong prefix đã duyệt, đọc cả hai. Audit-admin tải bản sao log `.json.gz`, chạy `Export-M12CloudTrailEvidence.ps1`.
...
2. **`gha-terraform-apply`** giữ `AdministratorAccess`, nhưng sau Phase 4b có boundary chặn các kill switch. **Phần còn lại:** `PutEventSelectors`, `UpdateTrail`, `PutBucketPolicy` vẫn được phép vì Terraform cần — một PR độc hại vẫn thu hẹp được selector hoặc gỡ Deny khỏi bucket policy. Bù bằng PR review + alert group 1/`g7` (đã bypass allowlist) + heartbeat so exact config mỗi 5 phút. Xem §9.3.
...
5. **`gitlab-ci-deployer`** vẫn trong allowlist tới PR IAM. Sau PR này, nhóm critical `[1,2,3,4,7,8]` đã luôn cảnh báo kể cả với principal này; phần còn bị bỏ qua là group 5 (đọc secret) và group 6 (huỷ hoại).
...
- [ ] `sns:Unsubscribe` đã verify resource-level binding; nếu không bắt thì ghi rõ và dựa vào `g7` + heartbeat
