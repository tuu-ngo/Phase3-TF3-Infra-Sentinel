...
> [!IMPORTANT]
> Bắt buộc đặt `PYTHONDONTWRITEBYTECODE=1`. `data.archive_file.audit_alert_router` zip **cả thư mục** `modules/audit-detection/lambda/`, nên `__pycache__/` do test sinh ra sẽ lọt vào artifact và làm `CodeSha256` lệch baseline → heartbeat FAIL sau apply.
...
Sau khi test, xác nhận thư mục nguồn sạch — nếu không, artifact router sẽ lệch baseline:
```powershell
Get-ChildItem -Recurse infra/modules/audit-detection/lambda   # chỉ được có index.py
```
...
Sau attach, user **không tự gỡ được** boundary: `additional_bounded_principal_arns` đưa ARN của nó vào statement `DenyRemovingOwnBoundary`, và statement đó nay bao gồm cả `iam:PutUserPermissionsBoundary`/`DeleteUserPermissionsBoundary`.

**Smoke test bắt buộc:** chạy một pipeline GitLab thật sau khi attach. Nếu hỏng, gỡ bằng `aws iam delete-user-permissions-boundary --user-name gitlab-ci-deployer` (từ identity MFA khác), root-cause rồi thử lại.
...
>
> Nguyên nhân số một của (2): `data.archive_file.audit_alert_router` dùng `source_dir = "${path.module}/lambda"`, nên **mọi file lạ trong thư mục đó đều lọt vào zip**. Chạy unit test §7 tạo `__pycache__/` ngay tại đó. Checklist điều tra:
>
> ```powershell
> # 1. Thư mục nguồn có file lạ không?
> Get-ChildItem -Recurse infra/modules/audit-detection/lambda
> # Chỉ được có index.py. Nếu thấy __pycache__ hoặc *.pyc: xoá, chạy lại plan/apply.
>
> # 2. So hash thật với baseline Terraform
> aws lambda get-function-configuration --function-name <router> --region ap-southeast-1 --query CodeSha256
> terraform -chdir=infra/live/production output -json m12_router_expected_debug
> ```
>
> Phòng ngừa: `.gitignore` đã loại `__pycache__/` và `*.py[cod]`; unit test §7 chạy với `PYTHONDONTWRITEBYTECODE=1`.
>
