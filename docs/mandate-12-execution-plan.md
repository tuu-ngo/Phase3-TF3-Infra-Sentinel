...
|---|---|---|---|
| 1 | **Exact S3 coverage được ký** | Data owner phân loại 8 bucket, chọn exact prefix nhạy cảm. Điền `audit_detection_s3_data_event_arns` (đang comment trong tfvars). Mỗi ARN kết thúc `/`, khớp 1:1 với coverage matrix.<br>**Gate này được Terraform cưỡng chế:** production truyền `require_s3_data_event_coverage = true`, nên `terraform plan` **FAIL** khi danh sách rỗng. Không thể apply nhầm một trail không ghi `GetObject` | ❌ |
...
> [!IMPORTANT]
> Từ khi thêm `require_s3_data_event_coverage = true`, **CI plan sẽ FAIL** với thông báo `audit_detection_s3_data_event_arns is empty` cho tới khi data owner điền ARN đã duyệt vào `production.auto.tfvars` (dòng đang comment). Đây là hành vi có chủ ý — gate coverage được Terraform cưỡng chế thay vì chỉ nằm trong checklist. Điền giá trị **trước** khi mở PR để review có plan xem.
...
> [!NOTE]
> **Lần heartbeat đầu sau apply có thể FAIL vì `CodeSha256`.** `source_code_hash` của Terraform (base64 SHA256 của zip do `archive_file` dựng) về nguyên tắc bằng `CodeSha256` Lambda trả về, nhưng nếu cơ chế đóng gói tạo khác biệt thì heartbeat báo lệch. Kiểm tra thủ công:
> ```powershell
> aws lambda get-function-configuration --function-name <router> --region ap-southeast-1 --query CodeSha256
> terraform -chdir=infra/live/production output -json | ConvertFrom-Json
> ```
> Nếu lệch do đóng gói chứ không phải do bị sửa code, bỏ riêng field `codeSha256` khỏi `m12_router_expected` và ghi lý do vào evidence. Ba field còn lại (`handler`, `roleArn`, `detectorConfig`) là chuỗi tất định, không có rủi ro này.
