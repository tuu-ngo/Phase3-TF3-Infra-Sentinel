...
- **Toàn cluster, trừ `kube-system`** — cả VAP lẫn PSA đều mở rộng ra ngoài `techx-tf3` (PR #343, 22/07), khớp đúng phạm vi Kyverno cũ (19-20/07). VAP dùng `namespaceSelector.matchExpressions: NotIn[kube-system]` (tự động áp cho namespace tạo mới sau này, không cần liệt kê tay như Kyverno). PSA `enforce=restricted` đã gán cho `techx-tf3`, `argocd`, `argo-rollouts`, `external-secrets`, `default`. Riêng `observability-system` **cố ý giữ `baseline`** (không nâng `restricted`) — exception có kiểm soát, vì `otel-node-agent` (DaemonSet host-metrics) cần `hostPath` để đọc `/proc`, `/sys` của node, việc mà `restricted` cấm hoàn toàn.
...
> **Lưu ý tên gọi:** tên object `mandate05-native-image-reference-techx-tf3` / `mandate05-native-resource-requirements-techx-tf3` (Binding) vẫn giữ hậu tố `-techx-tf3` từ lúc mới viết (trước khi mở rộng cluster-wide) — **đây chỉ là tên, không phản ánh phạm vi thật**. Phạm vi thật do `spec.matchResources.namespaceSelector` quyết định (mục 1), hiện là toàn cluster trừ `kube-system`. Không đổi tên lại để tránh phải cập nhật lại mọi tài liệu tham chiếu — mentor nên kiểm bằng `kubectl get validatingadmissionpolicybinding -o yaml` để thấy `namespaceSelector` thật, không suy ra phạm vi từ tên.

**Demo bổ sung — chứng minh cluster-wide thật, không chỉ `techx-tf3`:**

```bash
kubectl apply --dry-run=server -f docs/evidence/mandate-05/native-rejection-demo/bad-root-pod-cross-namespace.yaml
kubectl apply --dry-run=server -f docs/evidence/mandate-05/native-rejection-demo/bad-latest-image-pod-cross-namespace.yaml
```
2 fixture này **giống hệt** `bad-root-pod.yaml`/`bad-latest-image-pod.yaml` về nội dung vi phạm, nhưng đặt ở `namespace: default` thay vì `techx-tf3` — verify sống 22/07 (sau khi PR #343 merge): cả 2 đều bị chặn đúng bởi PSA/VAP, y hệt kết quả ở `techx-tf3`, xem mục 5.
...
**Kết quả demo cross-namespace (namespace `default`, sau khi PR #343 merge — không còn Kyverno tham gia vì đã Audit từ PR #339):**

```
$ kubectl apply --dry-run=server -f docs/evidence/mandate-05/native-rejection-demo/bad-root-pod-cross-namespace.yaml
Error from server (Forbidden): ... pods "mandate05-native-bad-root-cross-ns" is forbidden: violates
  PodSecurity "restricted:v1.35": allowPrivilegeEscalation != false ...

$ kubectl apply --dry-run=server -f docs/evidence/mandate-05/native-rejection-demo/bad-latest-image-pod-cross-namespace.yaml
The pods "mandate05-native-bad-latest-cross-ns" is invalid: : ValidatingAdmissionPolicy
  'mandate05-native-image-reference' with binding 'mandate05-native-image-reference-techx-tf3'
  denied request: Images must use an explicit non-latest tag or immutable sha256 digest.
```

Kết quả **giống hệt** bản gốc ở `techx-tf3` — xác nhận cluster-wide hoạt động thật, không phải chỉ đúng 1 namespace được test kỹ. Cũng đã verify `kube-system` (test lại `aws-load-balancer-controller`) **không** bị đụng — đúng đúng phạm vi loại trừ duy nhất.
