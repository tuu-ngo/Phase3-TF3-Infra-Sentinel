...
**Task liên quan:** PM-114, PM-104, PM-101, PM-122  
...
PM-114 phải đóng đúng đường bypass này bằng Kyverno `verifyImages` ở `Enforce`.

## 0.3. Gap phụ bắt buộc phải xử lý để PM-114 hoạt động thật
...
Task hiện tại đóng hai mảnh SBOM + PM-114. Không được tuyên bố toàn bộ Mandate 10 hoàn tất nếu action/base-image pinning và required-check evidence chưa được đóng ở task khác.
...
PM-114 bắt buộc chữ ký admission Enforce.
...
Sau PM-114 có thể thêm một verify-attestation rule riêng khi đã benchmark.
...
docs/runbooks/pm-114-enforce-cutover.md
docs/runbooks/pm-114-rollback.md
```
...
CI diff guard nên fail khi PR PM-114 thay đổi các khu vực này ngoài lý do tài liệu.
...
```text
PM-114/task complete; full Mandate 10 has remaining controls
```
...
- [ ] PM-114 marked complete only after mentor-verifiable rejection.
