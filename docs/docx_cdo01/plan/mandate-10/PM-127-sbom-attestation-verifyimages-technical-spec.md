...
**Master sequence:** `mandate-10-closure-execution-plan.md` Gates 2–3.
...
**Planning baseline mới nhất sau đồng bộ main:** `main@63db1d8d171a9d64284cea0f496a37b859791484`
...
### Latest-main planning re-audit tại `63db1d8`
...
- Có 7 workflow YAML và 37 external action references ở snapshot hiện tại; final DoD vẫn phải discover động toàn bộ workflow/reusable workflow.
...
- Sau khi đồng bộ main, render đủ bốn values tạo `/tmp/techx-prod-63db1d8.yaml` gồm 23,065 dòng và 30 image reference duy nhất. Inventory/allow-list phải được sinh lại từ output này; không giữ output 22,445 dòng của baseline cũ hoặc số của render ba values. Local `helm dependency build` cần cấu hình đủ năm Helm repository trước khi được dùng làm evidence, dù chart vendored hiện vẫn template được.
...
- Final evidence phải đính kèm exact baseline SHA, machine-generated workflow/action/Dockerfile/Helm inventories và Unicode format-control scan; số 7/37/28/30 chỉ là snapshot `63db1d8`.
...
```json
{
  ...
  "metadata": {
    "properties": [
      ...
      {
        "name": "techx.sourceSha",
        "value": "<40 hex>"
      },
      {
        "name": "techx.workflowRunId",
        "value": "<run id>"
      },
      {
        "name": "techx.workflowRunAttempt",
        "value": "<positive integer>"
      },
      {
        "name": "techx.generator",
        "value": "trivy"
      },
      {
        "name": "techx.generatorVersion",
...
Không nhúng `techx.sbomSha256` vào chính CycloneDX document vì đó là self-referential hash. Sau khi SBOM hoàn tất, workflow tính SHA-256 và lưu hash trong `sbom-index.json` cùng custom signed attestation type `https://github.com/tuu-ngo/Phase3-TF3-Infra-Sentinel/attestations/release-provenance/v1` chứa exact service/platform/source SHA/run ID/run attempt. Lookup chỉ nhận attestation match toàn bộ tuple đó; zero/nhiều match đều fail ambiguity.
...
├── release-provenance/
│   ├── <service>-linux-amd64.json
│   ├── <service>-linux-amd64.verify.jsonl
│   ├── <service>-linux-arm64.json
│   └── <service>-linux-arm64.verify.jsonl
├── signed-images.jsonl
└── sbom-index.json
```
...
RELEASE_PROVENANCE_TYPE="https://github.com/tuu-ngo/Phase3-TF3-Infra-Sentinel/attestations/release-provenance/v1"
: "${SOURCE_SHA:?missing SOURCE_SHA}"
: "${GITHUB_RUN_ID:?missing GITHUB_RUN_ID}"
: "${GITHUB_RUN_ATTEMPT:?missing GITHUB_RUN_ATTEMPT}"
: "${TRIVY_VERSION:?missing immutable TRIVY_VERSION}"
mkdir -p \
  release-evidence/sbom \
  release-evidence/attestations \
  release-evidence/release-provenance
...
for ((index = 0; index < service_count; index++)); do
  ...
  for platform in ${BUILD_PLATFORMS//,/ }; do
    ...
    jq \
      ...
      --arg generator_version "$TRIVY_VERSION" \
      '
        .metadata.properties =
          ((.metadata.properties // []) + [
            {"name":"techx.service","value":$service},
            {"name":"techx.platform","value":$platform},
            {"name":"techx.subjectDigest","value":$digest},
            {"name":"techx.sourceSha","value":$source_sha},
            {"name":"techx.workflowRunId","value":$run_id},
            {"name":"techx.workflowRunAttempt","value":$run_attempt},
            {"name":"techx.generator","value":"trivy"},
            {"name":"techx.generatorVersion","value":$generator_version}
          ])
      ' "$tmp" > "$sbom"
    ...
    provenance="release-evidence/release-provenance/${service}-${safe_platform}.json"
    ...
    cosign verify-attestation \
      ...
      --certificate-identity "$EXPECTED_IDENTITY" \
      "$image" \
      > "release-evidence/release-provenance/${service}-${safe_platform}.verify.jsonl"
...
```json
{
  ...
  "releaseProvenanceType": "https://github.com/tuu-ngo/Phase3-TF3-Infra-Sentinel/attestations/release-provenance/v1",
  "issuer": "https://token.actions.githubusercontent.com",
      ...
      "platforms": [
        {
          ...
          "sbomSha256": "<64 hex>",
          "generator": "trivy",
          "generatorVersion": "<immutable version>",
          "attestationVerifyPath": "attestations/checkout-linux-amd64.verify.jsonl",
          "releaseProvenancePath": "release-provenance/checkout-linux-amd64.json",
          "releaseProvenanceVerifyPath": "release-provenance/checkout-linux-amd64.verify.jsonl"
        },
        {
          ...
          "releaseProvenancePath": "release-provenance/checkout-linux-arm64.json",
          "releaseProvenanceVerifyPath": "release-provenance/checkout-linux-arm64.verify.jsonl"
        }
      ]
    }
  ]
...
6. chọn predicate khớp exact `techx.service`, `techx.platform`, `techx.subjectDigest`, `techx.sourceSha`, `techx.workflowRunId` và `techx.workflowRunAttempt`;
7. validate subject digest, generator/version và hash qua signed release-provenance/index contract;
8. in CycloneDX predicate ra stdout;
9. exit non-zero khi thiếu hoặc ambiguous; không tự chọn newest rerun/backfill.
...
PM-132 là source of truth cho final demo command. Run actual apply, not client-side validation, và không được coi mọi lỗi `kubectl get` là bằng chứng Pod absent:
...
test "$status" -ne 0
rg -F 'verify-first-party-signatures' \
  docs/evidence/mandate-10/final/unsigned-rejection.txt
rg -F 'verify-techx-main-workflow-signature' \
  docs/evidence/mandate-10/final/unsigned-rejection.txt

set +e
kubectl -n techx-tf3 get pod mandate10-unsigned-demo \
  > docs/evidence/mandate-10/final/unsigned-pod-absent.txt 2>&1
get_status=$?
set -e

test "$get_status" -ne 0
rg -qi 'notfound|not found' \
  docs/evidence/mandate-10/final/unsigned-pod-absent.txt
```
...
| SBOM-041 | service/source SHA/run ID/attempt metadata complete | Pass |
| SBOM-042 | generator/version missing or floating | Workflow fail |
| SBOM-043 | file hash stored only after final SBOM generation | Pass |
| SBOM-044 | signed release-provenance hash differs from index/file | Workflow fail |
...
| LOOK-010 | Multiple matching attestations | Exit ambiguous, even if payloads appear identical |
...
| LOOK-016 | Same digest/platform from different rerun attempts | Select exact run/attempt or exit ambiguous |
| LOOK-017 | Backfill and release attestations both present | Match exact source/run contract; no newest fallback |
...
- admission request duration p50/p95/p99;
- webhook request/error/timeout rate;
- registry lookup latency/error rate;
- CPU/memory;
- restart count;
- timeout count.

ADR phải chốt numeric latency budget trước Enforce từ baseline thật. Tối thiểu, controlled signed/unsigned test window phải có zero webhook timeout, zero registry/auth error và zero known-good rejection. Thiếu dashboard/query hoặc chưa chốt budget thì giữ Audit; không dùng từ “healthy” không có threshold làm exit gate.
...
Trước Enforce phải đánh giá và triển khai reliability control phù hợp:
...
Target là tối thiểu hai admission replicas, PDB `minAvailable: 1` và anti-affinity/topology spread qua node khi capacity cho phép. Đây phải là reviewed reliability change riêng, không lén gộp. Nếu capacity không cho phép, risk owner phải ký exception có expiry và rollback; không có acceptance thì Enforce vẫn `BLOCKED`.

Test outage bắt buộc trong môi trường an toàn phải ghi hành vi khi ECR/registry tạm unavailable. Expected security posture là fail closed trên image admission mới; runbook phải phân biệt deployment blocked với workload đang chạy và chỉ rollback qua GitOps commit đã chuẩn bị, không tạo bypass im lặng.
