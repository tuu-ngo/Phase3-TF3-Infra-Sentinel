...
1. **[Chẩn đoán]** Dựng 1 pod `checkout` MSK-env **cô lập** (không gắn Service, không traffic) → đọc
   log startup `CreateKafkaProducer` → xác định lỗi sarama chính xác. **Không** thử lại cutover tới
   khi biết root cause.
2. **[Fix code — bắt buộc] Fail-fast:** khi `KAFKA_ADDR` được set mà `CreateKafkaProducer` lỗi →
   checkout phải **fail readiness** (hoặc crash startup), tuyệt đối không phục vụ `PlaceOrder` với
   producer nil. Biến config-error thành "pod không Ready" (rollout đứng an toàn), không thành outage.
3. **[Fix code] Không nuốt lỗi produce:** `SendMessage` err ở `PlaceOrder` nên trả lỗi (để SLO/canary
   bắt được), thay vì chỉ log — hoặc ít nhất có metric/alert riêng cho produce-failure.
4. **[Fix nguyên nhân sarama]** Tùy kết quả (1): kiểm tra `IdempotentWrite` ACL cho SCRAM user / cân
   nhắc tắt `Idempotent` nếu MSK ACL chặn / kiểm TLS RootCAs / align `config.Version` với MSK 3.9.
5. **[Quy trình]** Với service gRPC + Argo Rollouts: canary **không** validate được đường client ở
   weight thấp (connection pinning). Cần chiến lược khác: test đường client bằng pod cô lập trước, và
   xem việc chuyển producer như thao tác "tất-cả-hoặc-không" có giám sát MSK offset + rollback nhanh.
6. Rebuild image checkout với fix → retry producer cutover.
