...
Trong lúc load-test/thử đặt đơn qua Locust (10 user), `POST /api/checkout` phản hồi rất chậm và fail nhiều, trong khi mọi endpoint khác (`/`, `/api/cart`) vẫn nhanh bình thường. Điều tra bằng Jaeger trace + Locust statistics + đọc code xác định nguyên nhân là 1 bug concurrency trong `checkout/main.go`: request bị "treo" chờ kết quả publish Kafka, và bị Envoy timeout mặc định 15s cắt ngang. Đây là hệ quả phụ (chưa từng được test dưới tải đồng thời) của chính fix REL-09 (đổi Kafka producer sang chờ ACK đồng bộ để tránh mất đơn âm thầm).

## Triệu chứng

Từ Locust (host `http://frontend-proxy:8080`, 10 users):
...
Biểu đồ Total Requests per Second của Locust cho thấy RPS rớt từ ~45 xuống ~2 tại 1 mốc và **không tự hồi phục** trong 4+ giờ quan sát tiếp theo — không phải nhiễu tạm thời, là hỏng vĩnh viễn kể từ lúc đó. 95th percentile response time dính cứng ở vùng 9000-15000ms liên tục suốt cửa sổ quan sát.

Trace Jaeger của 1 request `frontend-proxy: POST` (1.96s, 33 span, đánh dấu "Incomplete") cho thấy request đi qua `cart`, `checkout` (10 span — nhiều nhất), `currency`, `email`, `flagd`, `frontend-proxy`, `product-catalog`, `quote`, `shipping` — không phải bằng chứng định vị chính xác điểm treo (trace "Incomplete" nên một số span có thể chưa kịp flush lên Jaeger), nhưng xác nhận request đi hết một chuỗi service dài, khớp với luồng `PlaceOrder` thật trong code.
...
Fix REL-09 chỉ được test với traffic thấp/tuần tự — bug chỉ lộ ra khi có tải đồng thời thật (Locust nhiều user cùng lúc), điều mà quy trình verify trước đó của REL-09 chưa bao phủ.

## Rủi ro nếu không xử lý

Đây là service ra tiền, nằm ngay trên SLO chấm điểm chính (checkout ≥99%). Ở tải cao hơn 200 user (mục tiêu Mandate #2), tỷ lệ request đồng thời trên mỗi pod checkout sẽ còn cao hơn 10-user hiện tại, khiến tỷ lệ % request bị "cướp" tín hiệu tăng theo — bug này gần như chắc chắn làm fail bài load-test 200 user/15 phút và vi phạm SLO ngay trong buổi chấm nếu không sửa trước.
...
- [ ] Push code, để CI build+push image `checkout` mới (scoped build tự động theo service đổi).
- [ ] Bump `imageOverride.tag` cho `checkout` trong `deploy/values-prod.yaml`, tạo PR, merge, deploy qua ArgoCD.
- [ ] Chạy lại Locust (tối thiểu vài chục user đồng thời) để xác nhận `/api/checkout` hết dính 15s và tỷ lệ fail về 0.
- [ ] Verify trace Jaeger của 1 request checkout thật sau fix để thấy đủ span `payment`/`kafka publish`, không còn "Incomplete" bất thường.
