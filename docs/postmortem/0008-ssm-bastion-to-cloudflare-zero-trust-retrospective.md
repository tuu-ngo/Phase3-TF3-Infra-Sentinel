...
1. **Rotate Cloudflare API token** sau bài tập + xoá file token trong `~/Downloads` (rủi ro đang
   ghi CLAUDE.md).
2. **Chuyển domain** từ `arthur-ngo.org` (cá nhân) sang domain tổ chức khi có.
3. **Quyết định số phận bastion:** giữ break-glass hay gỡ — sau khi đường Cloudflare đủ chín, ra
   quyết định có ký (ADR) thay vì để lửng.
4. **(Cân nhắc) allowlist theo `email_domain`** thay vì liệt kê từng email — module đã hỗ trợ
   `cloudflare_allowed_email_domain`. *(Quyết định hiện tại: giữ liệt kê từng email để kiểm soát
   chặt từng người.)*
5. **Nâng REL-17 thành ADR có ký** — quyết định này đủ lớn (trust anchor mới + đổi mô hình access)
