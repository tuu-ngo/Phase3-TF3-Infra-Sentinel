...
# ── Mandate 08: Managed Datastores ──────────────────────────────────────────

output "rds_endpoint" {
  description = "RDS PostgreSQL endpoint (host:port). Connect via internal VPC DNS — not public."
  value       = module.managed_datastores.rds_endpoint
}

output "rds_secret_arn" {
  description = "Secrets Manager ARN for the RDS master credentials (generated by RDS)."
  value       = module.managed_datastores.rds_secret_arn
}

output "redis_primary_endpoint" {
  description = "ElastiCache Redis primary endpoint (TLS, port 6380)."
  value       = module.managed_datastores.redis_primary_endpoint
}

output "redis_auth_secret_arn" {
  description = "Secrets Manager ARN for the Redis AUTH token."
  value       = module.managed_datastores.redis_auth_secret_arn
}

output "msk_bootstrap_brokers_tls" {
  description = "MSK bootstrap brokers for SASL/SCRAM clients (TLS, port 9094)."
  value       = module.managed_datastores.msk_bootstrap_brokers_tls
}

output "msk_bootstrap_brokers_iam" {
  description = "MSK bootstrap brokers for IAM-authenticated clients (port 9098)."
  value       = module.managed_datastores.msk_bootstrap_brokers_iam
}

output "msk_scram_secret_arn" {
  description = "Secrets Manager ARN for the MSK SASL/SCRAM credential. Populate before enabling aws_msk_scram_secret_association."
  value       = module.managed_datastores.msk_scram_secret_arn
}

output "datastore_secrets_read_policy_arn" {
  description = "Attach this IAM policy ARN to IRSA roles that need to read datastore credentials."
  value       = module.managed_datastores.datastore_secrets_read_policy_arn
}

output "msk_iam_auth_policy_arn" {
  description = "Attach this IAM policy ARN to IRSA roles for MSK IAM-based Kafka auth."
  value       = module.managed_datastores.msk_iam_auth_policy_arn
}
